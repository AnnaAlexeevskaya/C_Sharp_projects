...
// расчет суммы 1 методом

int[] SumPostiveNegativeElem(int[] arr)
{
int sumPostive = 0;
int SumNegative = 0;
for (int i = 0; i< arr.Length; i++)
{
  if (arr[i]>0) sumPostive +=arr[i];
  else SumNegative+=arr[i];  
}

return new int[] {sumPostive, SumNegative};
}

int[] sumPostiveNegativeElem = SumPostiveNegativeElem(array);

Console.WriteLine($"Сумма положительных элементов равна {sumPostiveNegativeElem[0]}, Сумма отрицательных элементов равна {sumPostiveNegativeElem[1]}");

//решение через 2 отдельных метода
